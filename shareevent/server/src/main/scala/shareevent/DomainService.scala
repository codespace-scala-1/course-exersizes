...
trait DomainService {
  ...
  def schedule(eventId: Event.Id, locationId: Location.Id, time: DateTime): DomainContext => Try[ScheduleItem]
  ...
  def run(confirmation: Confirmation): DomainContext => Try[Event]
